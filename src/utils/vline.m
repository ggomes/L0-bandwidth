...
%    h=plot([x x],y,linetype,'LineWidth',2);
    ...
    if length(label)
        ...
        xrange=xx(2)-xx(1);
        xunit=(x-xx(1))/xrange;
        if xunit<0.95 % originally was 0.8
            text(x+0.015*xrange,y(1)+0.9*(y(2)-y(1)),label,'color',get(h,'color'),'Rotation',90,'FontWeight','bold')
        else
            text(x-.05*xrange,y(1)+0.9*(y(2)-y(1)),label,'color',get(h,'color'),'Rotation',90,'FontWeight','bold')
        end
    end
